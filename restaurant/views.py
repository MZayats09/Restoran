...
def booking_map(request):
    ...
    for t in tables:
        ...
        t.svg_x = round(t.pos_x * 10)
        t.svg_y = round(t.pos_y * 6)
        size = 46 + t.seats * 9
        t.svg_size = size
        t.rect_x = round(t.svg_x - size / 2)
        t.rect_y = round(t.svg_y - size / 2)
        t.circle_r = round(size / 2)

    window_ys = [40, 120, 200, 280, 360, 440, 520]
...
# ---------- Панель менеджера: черга замовлень і сповіщення ----------

def _is_manager(user):
    return user.is_authenticated and user.is_staff


@user_passes_test(_is_manager, login_url='login')
def manager_orders(request):
    orders = Order.objects.exclude(status='done').prefetch_related('items').order_by('created_at')
...
def order_set_status(request, pk, status):
    ...
    order.status = status
    if status == 'ready':
        order.ready_notified = False  # клієнту ще належить показати сповіщення
    order.save()
    messages.success(request, f'Замовлення №{order.pk} → «{valid_statuses[status]}».')
    return redirect('manager_orders')


@login_required
def notifications_check(request):
    """Polled from the navbar: returns orders that just became ready and
    marks them as seen so the toast is shown only once per client."""
    ready_orders = Order.objects.filter(
        user=request.user, status='ready', ready_notified=False
    )
    data = [{'id': o.pk, 'number': o.pk} for o in ready_orders]
    ready_orders.update(ready_notified=True)
